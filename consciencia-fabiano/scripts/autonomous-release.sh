...
log "== Consciência do Fabiano :: V3.1 OFFLINE TURBINES release =="
...
node -e 'const fs=require("fs");const m=JSON.parse(fs.readFileSync("public/failover-manifest.json","utf8"));if(m.version!=="3.1.0")process.exit(1)'
node -e 'const fs=require("fs");const s=JSON.parse(fs.readFileSync("public/steel/index.json","utf8"));if(s.version!=="3.1.0")process.exit(1)'
...
jq -e '.version == "3.1.0" and .strategy == "A->B->C->D->E->F" and .plan_c.logical_task_capacity == 1000 and .plan_c.physical_worker_cap == 16 and .plan_c.virtualized_cards == true and .plan_c.card_gap_px == 40' /tmp/failover-manifest.json >/dev/null || die "FAILOVER_MANIFEST_BAD"
curl -fsS "$BASE/steel/index.json" | tee /tmp/steel-index.json >/dev/null
jq -e '.version == "3.1.0" and (.shards|type) == "array"' /tmp/steel-index.json >/dev/null || die "STEEL_INDEX_BAD"
