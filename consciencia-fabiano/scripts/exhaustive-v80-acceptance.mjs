...
assert(index.includes("function exhaustiveStructureGate"),"structural-multisection-gate");
assert(index.includes("structure?.sufficient===false"),"structural-gate-can-force-continuation");
assert(index.includes("structuralOnly=!missing.length && structure.sufficient===false"),"single-block-coverage-does-not-stop-continuation");
...
assert(index.includes('code:"MEMORY_ROUTE_DEGRADED"'),"memory-route-never-500-guard");
assert(index.includes('code:"R2_RECONCILE_ROUTE_DEGRADED"'),"r2-route-never-500-guard");
