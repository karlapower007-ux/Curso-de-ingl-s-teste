...
assert.ok(ui.includes('browserOnly&&!localApiConnected()'),"queda do Ollama não pode forçar fallback frouxo quando o servidor local está disponível");
assert.ok(ui.includes("function localApiConnected()"),"ponte local em mesma origem deve ser reconhecida mesmo quando apiBase é string vazia");
...
assert.ok(ui.includes('mode==="exact"&&engine&&Number(localState?.chunks||0)>0&&!localApiConnected()'),"Citação exata só pode bypassar o servidor quando a ponte local não existir");
