(() => {
  ...
  function createNodeProgressVirtualizer(host) {
    ...
    return {
      upsert(data){
        ...
        header.textContent="RAG V4.0 • "+Math.min(record.completed,NODE_VIRTUAL_MAX)+"/"+NODE_VIRTUAL_MAX+" nós concluídos"+
  ...
  function attachCitationDictionary(messageWrap,query){
    ...
    const state={
      ...
      citations:[],
      ids:new Set(),
      carry:{document_id:"",chapter_number:null,chapter_title:"",scripture:null},
      generation:""
    };

    const progressText=()=>{
      const found=state.citations.length.toLocaleString("pt-BR");
    ...
    const updateToggle=()=>{
      const suffix=state.scanDone ? " encontrados" : "+ encontrados";
      toggle.textContent="📚 Dicionário de Citações — "+state.citations.length.toLocaleString("pt-BR")+suffix;
    };

    const renderPage=(offset)=>{
      const maxOffset=Math.max(0,Math.floor(Math.max(0,state.citations.length-1)/CITATION_UI_PAGE_SIZE)*CITATION_UI_PAGE_SIZE);
      state.offset=Math.min(Math.max(0,Number(offset||0)),maxOffset);
      const rows=state.citations.slice(state.offset,state.offset+CITATION_UI_PAGE_SIZE);
      list.replaceChildren();
      for(const item of rows) list.appendChild(renderCitationCard(item));
      if(!rows.length && state.scanDone){
      ...
      pageInfo.textContent=state.scanDone
        ? start+"–"+end+" de "+state.citations.length.toLocaleString("pt-BR")
        : start+"–"+end+" de "+state.citations.length.toLocaleString("pt-BR")+"+";
      prev.disabled=state.offset<=0 || state.loading;
      next.disabled=state.loading || (state.scanDone && end>=state.citations.length);
      updateToggle();
    };
    ...
    const segmentUrl=()=>{
      const params=new URLSearchParams({
        q:query,
        limit:String(CITATION_UI_PAGE_SIZE)
      });
      if(state.scanCursor) params.set("scan_cursor",state.scanCursor);
      ...
      if(state.carry?.scripture?.work) params.set("carry_scripture_work",state.carry.scripture.work);
      if(state.carry?.scripture?.book) params.set("carry_scripture_book",state.carry.scripture.book);
      if(state.carry?.scripture?.chapter) params.set("carry_scripture_chapter",String(state.carry.scripture.chapter));
      if(state.carry?.scripture?.verse_start) params.set("carry_scripture_verse_start",String(state.carry.scripture.verse_start));
      if(state.carry?.scripture?.verse_end) params.set("carry_scripture_verse_end",String(state.carry.scripture.verse_end));
      if(state.carry?.scripture?.reference) params.set("carry_scripture_reference",state.carry.scripture.reference);
      return "/api/citations?"+params.toString();
    };

    const scanOneSegment=async()=>{
      if(state.scanDone) return false;
      const beforeCursor=state.scanCursor;
      const res=await fetch(segmentUrl(),{cache:"no-store"});
      const data=await res.json().catch(()=>({}));
      if(!res.ok || data?.ok===false) throw new Error(data?.message || data?.code || "Dicionário de citações indisponível.");

      state.libraryTotal=Math.max(state.libraryTotal,Number(data?.library_total_chunks||0));
      state.scanned+=Math.max(0,Number(data?.scanned_batch ?? data?.scanned ?? 0));
      state.generation=String(data?.generation||state.generation||"");
      state.carry=data?.carry && typeof data.carry==="object" ? data.carry : state.carry;

      const rows=Array.isArray(data?.citations)?data.citations:[];
      for(const raw of rows){
        const id=String(raw?.citation_id || raw?.chunk_id || (raw?.document_id+"-"+raw?.chunk_index));
        if(state.ids.has(id)) continue;
        state.ids.add(id);
        state.citations.push({...raw,rank:state.citations.length+1});
      }

      state.scanDone=Boolean(data?.scan_done);
      state.scanCursor=state.scanDone ? "" : String(data?.next_scan_cursor||"");
      if(!state.scanDone && !state.scanCursor){
        throw new Error("A varredura de citações não forneceu cursor de continuação.");
      }
      if(!state.scanDone && beforeCursor && state.scanCursor===beforeCursor){
        throw new Error("A varredura de citações não avançou.");
      }
      return true;
    };

    const ensureRows=async(targetCount)=>{
      const target=Math.max(0,Number(targetCount||0));
      while(!state.scanDone && state.citations.length<target){
        await scanOneSegment();
        renderPage(state.offset);
        await new Promise(resolve=>setTimeout(resolve,35));
      }
    };

    const continueBackgroundScan=async()=>{
      if(state.loading || state.scanDone || !state.open) return;
      state.loading=true;
      try{
        while(state.open && !state.scanDone){
          await scanOneSegment();
          renderPage(state.offset);
          await new Promise(resolve=>setTimeout(resolve,90));
        }
      }catch(error){
        status.textContent="A varredura foi pausada sem perder as citações já encontradas: "+String(error?.message||error);
      }finally{
        state.loading=false;
        renderPage(state.offset);
      }
    ...
    const loadPage=async(offset)=>{
      ...
      status.textContent=state.loaded
        ? "Carregando o próximo bloco de referências…"
        : "Buscando referências bibliográficas na biblioteca preservada…";
      prev.disabled=true;
      next.disabled=true;
      try{
        await ensureRows(safeOffset+CITATION_UI_PAGE_SIZE);
        state.loaded=true;
        renderPage(safeOffset);
      }catch(error){
        list.replaceChildren();
        const failure=document.createElement("div");
        failure.className="citation-empty";
        failure.textContent="Não foi possível continuar o Dicionário agora: "+String(error?.message||error);
        list.appendChild(failure);
        status.textContent="As citações já localizadas foram preservadas. Tente novamente para continuar a partir do mesmo ponto.";
        pageInfo.textContent="Varredura pausada";
      }finally{
        state.loading=false;
        renderPage(state.offset);
      }
      if(state.open && !state.scanDone) setTimeout(()=>continueBackgroundScan(),120);
    };
    ...
    toggle.addEventListener("click",()=>{
      ...
      else if(state.open && !state.scanDone) setTimeout(()=>continueBackgroundScan(),80);
    });
  ...
  function sourceStem(item){
    return String(item?.source_title || item?.filename || item?.title || "Documento")
      .replace(/\.pdf$/i,"").trim() || "Documento";
  }

  function canonicalHeader(item){
    const ref=String(item?.canonical_reference||"").trim();
    const source=sourceStem(item);
    return ref ? ref+" ("+source+")" : String(item?.semantic_title || item?.title || source);
  }

  function appendTextWithoutDuplicate(left,right){
    ...
    const max=Math.min(900,a.length,b.length);
  ...
  function mergeSequentialOfflineCards(input){
    const cards=Array.from(input||[]).map((card,rank)=>({...card,__rank:rank}));
    const groups=new Map();
    for(const card of cards){
      const key=String(card?.document_id || card?.filename || card?.source_title || "unknown");
    ...
    for(const rows of groups.values()){
      rows.sort((a,b)=>Number(a.chunk_index||0)-Number(b.chunk_index||0)||a.__rank-b.__rank);
      let current=null;
      for(const row of rows){
        const idx=Number(row.chunk_index);
        if(!current){
          current={...row,chunk_start:idx,chunk_end:idx,merged_chunks:[idx],references:[row.canonical_reference].filter(Boolean)};
          continue;
        }
        const sameDocument=String(current.document_id||current.filename||current.source_title||"")===String(row.document_id||row.filename||row.source_title||"");
        const sequential=Number.isFinite(idx)&&idx===Number(current.chunk_end)+1;
        if(sameDocument&&sequential){
          current.text=appendTextWithoutDuplicate(current.text,row.text);
          current.chunk_end=idx;
          current.merged_chunks.push(idx);
          current.score=Math.max(Number(current.score||0),Number(row.score||0));
          current.coverage=Math.max(Number(current.coverage||0),Number(row.coverage||0));
          current.page=current.page||row.page||null;
          current.__rank=Math.min(current.__rank,row.__rank);
          if(row.canonical_reference&&!current.references.includes(row.canonical_reference))current.references.push(row.canonical_reference);
          if(!current.canonical_reference&&row.canonical_reference)current.canonical_reference=row.canonical_reference;
          current.semantic_title=canonicalHeader(current);
        }else{
          merged.push(current);
          current={...row,chunk_start:idx,chunk_end:idx,merged_chunks:[idx],references:[row.canonical_reference].filter(Boolean)};
        }
    ...
    merged.sort((a,b)=>Number(b.score||0)-Number(a.score||0)||a.__rank-b.__rank);
    return merged.map(({__rank,...card})=>card);
  }
  ...
  function buildOfflineTurbineCard(item,index){
    ...
    const title=document.createElement("strong");
    const elevated=String(item?.canonical_reference||item?.semantic_title||"").trim();
    title.textContent=elevated || sourceStem(item);
  ...
  function appendOfflineTurbineResults(data){
    ...
    const cards=mergeSequentialOfflineCards(rawCards);
    const wrap=document.createElement("div");
    ...
    const schedule=()=>{
      ...
      raf=requestAnimationFrame(()=>{
        ...
        const top=Math.max(0,viewport.scrollTop-760);
        const bottom=viewport.scrollTop+Math.max(360,viewport.clientHeight||520)+760;
        const fragment=document.createDocumentFragment();
  ...
  function appendElegantSilence(message="Nenhuma correspondência exata encontrada na biblioteca total.") {
    ...
    text.textContent=String(message||"Nenhuma correspondência exata encontrada na biblioteca total.");
    wrap.appendChild(text);
    $("messages").appendChild(wrap);
    $("messages").scrollTop=$("messages").scrollHeight;
    return wrap;
