const FNS_STT_URL='https://fns-stt.karlapower007.workers.dev/stt';
const teachers=[
...
function live(){layout(`<h1>Prática ao vivo</h1><p>Katya usa LiveAvatar. Os outros nove professores usam FNS Lite: reconhecimento de voz + voz do navegador + tutor pedagógico local, sem API key e sem custo.</p><div class="grid">${teachers.map((t,i)=>`<div class="card teacher"><span class="tag">${t.provider}${t.premium?' • PREMIUM':' • GRATUITO'}</span><h3>${t.name}</h3><div>${t.accent} English</div><p class="small muted">${t.premium?'Avatar premium em tempo real.':'Conversa por voz e texto, com correção pedagógica local.'}</p><button class="primary" onclick="openTeacher(${i})">Abrir professor</button></div>`).join('')}</div>`)}
...
function pickVoice(){if(!voices.length)loadVoices();const lang=activeTeacher.accent==='British'?'en-GB':'en-US';let candidates=voices.filter(v=>v.lang&&v.lang.toLowerCase().startsWith(lang.toLowerCase().slice(0,2)));if(activeTeacher.accent==='British'){const gb=voices.filter(v=>v.lang==='en-GB');if(gb.length)candidates=gb}else{const us=voices.filter(v=>v.lang==='en-US');if(us.length)candidates=us}if(!candidates.length)candidates=voices.filter(v=>v.lang&&v.lang.startsWith('en'));return candidates[(activeTeacher.i||0)%Math.max(1,candidates.length)]}
let lastSpoken='';
let voiceUnlocked=false;
function unlockVoice(){
  if(!('speechSynthesis' in window)){
    addMsg('system','Este navegador não oferece síntese de voz.');
    return;
  }
  voiceUnlocked=true;
...
function unlockAndRepeat(){
  voiceUnlocked=true;
  try{speechSynthesis.cancel();speechSynthesis.resume()}catch(e){}
  if(lastSpoken)speakNow(lastSpoken);
}
function speak(text){
  lastSpoken=text;
...
function speakNow(text){
  ...
  u.lang=activeTeacher&&activeTeacher.accent==='British'?'en-GB':'en-US';
  const v=pickVoice();
  if(v)u.voice=v;
  u.rate=activeTeacher&&activeTeacher.accent==='British'?.94:.97;
  u.pitch=activeTeacher&&activeTeacher.gender==='male'?.92:1.04;
  u.volume=1;
  u.onstart=()=>{
    setStatus('Speaking','busy');
    if(document.querySelector('#avatarFace'))avatarFace.className='avatar-face avatar-speaking';
  };
  u.onend=()=>{
    setStatus('Ready','on');
    if(document.querySelector('#avatarFace'))avatarFace.className='avatar-face';
  ...
  u.onerror=e=>{
    setStatus('Voice blocked','busy');
    addMsg('system','A voz do navegador foi bloqueada. Clique novamente em 🔊 Ativar voz ou 🔁 Repetir.');
  };
