...
log "== Consciência do Fabiano :: V3.2 SEMANTIC EXPANSION release =="
...
node -e 'const fs=require("fs");const m=JSON.parse(fs.readFileSync("public/failover-manifest.json","utf8"));if(m.version!=="3.2.0")process.exit(1)'
node -e 'const fs=require("fs");const s=JSON.parse(fs.readFileSync("public/steel/index.json","utf8"));if(s.version!=="3.2.0")process.exit(1)'
...
jq -e '.version == "3.2.0" and .strategy == "A->B->C->D->E->F" and .plan_c.logical_task_capacity == 1000 and .plan_c.physical_worker_cap == 16 and .plan_c.virtualized_cards == true and .plan_c.card_gap_px == 40 and .plan_c.window_expansion.before == 2 and .plan_c.window_expansion.after == 4 and .plan_c.window_expansion.full_chunk_fallback == true and .plan_c.sequential_chunk_merge == true and .plan_c.canonical_reference_elevation == true' /tmp/failover-manifest.json >/dev/null || die "FAILOVER_MANIFEST_BAD"
curl -fsS "$BASE/steel/index.json" | tee /tmp/steel-index.json >/dev/null
jq -e '.version == "3.2.0" and (.shards|type) == "array"' /tmp/steel-index.json >/dev/null || die "STEEL_INDEX_BAD"
